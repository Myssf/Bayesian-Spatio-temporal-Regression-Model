# setwd("E:/R/Group/Code/Simulation/")
...
sourceCpp("step1.cpp")
#sourceCpp("/work/chaom/ST_Sim/step1.cpp")
...
#####################################
#   df.PCR$Z_i <- 0
#for (i in 1:nrow(df.PCR)) {
#  if (df.PCR$Pool.size[i] == 1 && df.PCR$PanR[i] == "Positive") {
#    df.PCR$Z_i[i] <- 1
#  } else if (df.PCR$Pool.size[i] > 1 && df.PCR$PanR[i] == "Positive") {
#    for (j in 1:df.PCR$Pool.size[i]) { {
#      df.PCR$Z_i[i + j - 1] <- 1
#    }
#    }
#  }
...
#HypePanRrameter for sensitivity and specificity
a.sens <- 1 #0.095
b.sens <- 1 #0.005
a.spec <- 1 #0.095
b.spec <- 1 #0.005
...
Z <- c(Z,Y[unlist(pool_list[which(Z==1)])])
Z.tilde <- c(Z.tilde,Y_tilde.true[unlist(pool_list[which(Z==1)])])
length(Z)
sum(Z)

n.tests <- length(Z)
#Update lists
assay.pool.list <- append(assay.pool.list, assay.ind.list[unlist(pool_list[which(Z==1)])])
...
pool_list <- append(pool_list, unlist(pool_list[which(Z==1)]))
...
sej.mcmc <- spj.mcmc <- matrix(NA, n.iter, n.tests)

sej.mcmc[1,] <- sej <- sens.pool.list 
spj.mcmc[1,] <- spj <- spec.pool.list

sej <- rep(sens.init,n.tests)
spj <- rep(spec.init,n.tests)
...
for (t in 2:n.iter){
  ...
  cat("| I:", t, 
      "| l_s :", round(ell_s.mcmc[t], 2), sprintf("(%.2f%%)", 100 * ell_s.acc.count / t),
      "| l_t :", round(ell_t.mcmc[t], 2), sprintf("(%.2f%%)", 100 * ell_t.acc.count / t),
      "| sigma :", round(sigma.sq.mcmc[t], 2),
...
if (pred.from == "Yes") {
  ...
  for (i in 1:burn.in) {
    ...
    xi.grid_i <- xi.grid_i - mean(xi.grid_i)  # Center
...
#Plot for Sensitivity and Specifity
...
# Create bias map for year 4
#grid.sf <- grid_list[["4"]]
#coords <- st_coordinates(grid.sf)
#grid.sf$long <- coords[, "X"]
#grid.sf$lat <- coords[, "Y"]
#grid.sf$p.grid.bias <- p.grid.diff

#bias_range <- max(abs(min(p.grid.diff, na.rm = TRUE)), abs(max(p.grid.diff, na.rm = TRUE)))

#plot.bias4 <- ggplot() +
#  geom_sf(data = grid.sf, aes(colour = p.grid.bias), shape = 15, size = 3) +
# scale_colour_gradient2(low = "blue", mid = "white", high = "red", 
#                        midpoint = 0, limits = c(-bias_range, bias_range), 
#                        name = "Bias (True - Predicted)") +
#coord_sf(xlim = c(min.long, max.long), ylim = c(min.lat, max.lat), expand = FALSE) +
#geom_sf(data = shp.sf, fill = NA, color = "black", lwd = 0.3) +
#theme_bw() +
#ggtitle("Bias in Predicted Prevalence for Year 4")

# Store bias plot
#plot.bias_list <- list()
#plot.bias_list[["4"]] <- plot.bias4
