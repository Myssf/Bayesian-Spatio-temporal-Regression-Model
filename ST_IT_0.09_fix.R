...
#HypeRrameter for sensitivity and specificity
a.sens <- 1 #0.095
b.sens <- 1 #0.005
a.spec <- 1 #0.095
b.spec <- 1 #0.005
...
if(df.sim$Protocol[sim.ind] == "IT"){
  Z <- Y
  pool_list <- as.list(1:N)  # each individual is its own pool
  n.tests <- N
}
...
sej.mcmc[1,] <- sej <- sens.pool.list # Initialize with true sensitivities
spj.mcmc[1,] <- spj <- spec.pool.list # Initialize with true specificities
...
for (yr in total_years) {
  ...
  # xi.grid.true
  xi.grid.true_yr <- xi.grid.true_list[[as.character(yr)]]
...
for (yr in total_years) {
  ...
  p.grid.true_yr <- p.grid.true_list[[as.character(yr)]]
  
  p.grid.diff <- as.vector(p.grid.true_yr - p.grid.hat_list[[yr]])
  p.grid.mse <- p.grid.diff^2 + p.grid.sd^2
  p.grid.cover <- as.vector(ifelse(p.grid.true_yr >= p.grid.low & p.grid.true_yr <= p.grid.upp, 1, 0))
  assign(paste0("p.grid.out_", yr), rbind(p.grid.diff, p.grid.cover, p.grid.hat_list[[as.character(yr)]], p.grid.sd, p.grid.mse))
}
